 ...
 * running background threads and (optionally) randomly failing some number of them.
 */
public class InRamMemoryApi implements MemoryApi {
  ...
  /**
   * Set a random rate at which API operations will fail and throw exceptions, so we can test the
   * app against a simulated unreliable service or network connection.
   *
  ...
  private class GetMemoryTask extends AsyncTask<AsyncParams, Void, AsyncResult> {
    @Override
    protected void onPostExecute(AsyncResult asyncResult) {
      if (asyncResult.exception != null) {
    ...
    protected AsyncResult doInBackground(AsyncParams... asyncParamses) {
      ...
      if (!list.containsKey(params.memoryDate)) {
        result.exception = new MyDeticNoMemoryFoundException(params.userId, params.memoryDate);
      }
  ...
  /** Background task to fetch a single memory */
  private class PutMemoryTask extends AsyncTask<AsyncParams, Void, AsyncResult> {
    @Override
    protected void onPostExecute(AsyncResult asyncResult) {
      if (asyncResult.exception != null) {
        asyncResult.singleMemoryListener.onApiError(asyncResult.exception);
  ...
  /** Background task to fetch a single memory */
  private class DeleteMemoryTask extends AsyncTask<AsyncParams, Void, AsyncResult> {
    @Override
    protected void onPostExecute(AsyncResult asyncResult) {
      if (asyncResult.exception != null) {
        asyncResult.singleMemoryListener.onApiError(asyncResult.exception);
