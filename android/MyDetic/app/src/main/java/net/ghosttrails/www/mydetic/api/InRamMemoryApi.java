...
public class InRamMemoryApi implements MemoryApi {
     ...
     */
    @Override
    public MemoryData getMemory(String userId, Date memoryDate) throws NoMemoryFoundException {
     ...
     */
    @Override
    public MemoryData deleteMemory(String userId, Date memoryDate) throws NoMemoryFoundException {
        Map<Date, MemoryData> list = getListForUserId(userId);
        if(!list.containsKey(memoryDate)) {
            throw new NoMemoryFoundException(userId, memoryDate);
        } else {
