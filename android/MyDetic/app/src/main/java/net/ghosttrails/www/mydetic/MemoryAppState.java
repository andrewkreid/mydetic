...
public class MemoryAppState implements MemoryAppInterface {
  ...
  public static synchronized MemoryAppState getInstance() {
    if (_state == null) {
  ...
  /**
   * Fetch a memory from the cache if one exists. As the app loads memories, they are stored in this
   * cache to avoid unnecesary network calls.
   *
  ...
  public MemoryData getCachedMemory(LocalDate d) {
    ...
    if ((memory == null) && (dbHandle != null)) {
      // Try the DB cache.
      memory =
          MyDeticSQLDBContract.getMemory(
              dbHandle, config.getUserName(), config.getActiveDataStore(), d);
    }
    return memory;
  }
  ...
  void loadMemoryDatesFromApi(
      final Context context, final MemoryApi.MemoryListListener externalListener) {
    getApi()
        .getMemories(
            config.getUserName(),
            new MemoryApi.MemoryListListener() {
              ...
              public void onApiResponse(MemoryDataList newMemories) {
                try {
                  MyDeticSQLDBContract.putMemoryDates(
                      dbHandle, config.getUserName(), config.getActiveDataStore(), newMemories);
  ...
  /**
   * Update the API and user params from the config (eg when it has changed). Clear existing memory
   * data in RAM.
   */
